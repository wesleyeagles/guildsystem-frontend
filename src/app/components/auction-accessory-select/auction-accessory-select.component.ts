...
function lower(s: string) { return asStr(s).toLowerCase(); }
...
function unitKind(label: string): UnitKind { return EFFECT_UNIT_BY_LABEL[asStr(label)] ?? 'none'; }
...
function effectsChipsFromItem(it: AuctionCatalogItem): EffectChip[] {
  const fx = (it.effects ?? [])
    ...
    .map((e) => ({ text: `${asStr(e.label)} ${formatEffectValueFromCatalog(e)}`.trim() }));
...
@Component({
  selector: 'app-auction-accessory-select',
  standalone: true,
  ...
  template: `
    <div class="space-y-1">
      ...
      <div class="relative" #root>
        ...
        >
          @if (selectedItem()) {
            <div class="w-14 h-14 rounded-lg bg-slate-950 border border-slate-800 overflow-hidden flex items-center justify-center shrink-0">
              @if (selectedImg()) { <img [src]="selectedImg()!" class="w-full h-full object-cover p-2" /> }
            ...
            <div class="min-w-0 flex-1 text-left"><div class="text-slate-300">Selecione um acessório...</div></div>
          }
        ...
        @if (open()) {
          <div class="absolute z-[80] mt-2 w-full rounded-2xl bg-slate-950 border border-slate-800 shadow-xl overflow-hidden"
            ...
            <div class="p-2 border-b border-slate-800">
              ...
              <div class="mt-2 flex items-center justify-between text-xs text-slate-500 px-1">
                ...
                <button type="button" class="text-slate-300 hover:text-white disabled:opacity-50"
                        (click)="clear()" [disabled]="!selectedRef()">
                  Limpar
                </button>
              </div>
            </div>
...
export class AuctionAccessorySelectComponent {
  ...
  constructor() {
    this.query$
      .pipe(
        ...
        switchMap((q) => this.catalog.accessories(q, null, 60)),
  ...
  private ensureInitialLoaded() {
    ...
    this.loading.set(true);
    this.catalog.accessories('', null, 60).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (res) => {
        this.itemsSig.set(res.items ?? []);
        this.cursor.set(res.nextCursor ?? null);
        this.hasMore.set(Boolean(res.nextCursor));
  ...
  loadMore() {
    ...
    this.loading.set(true);
    this.catalog.accessories(this.query(), cur, 60).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (res) => {
  ...
  selectedItem = computed(() => {
    ...
    return this.itemsSig().find((x) => x.itemType === 'accessory' && x.itemId === id) ?? null;
  });
  ...
  chipsAll(it: AuctionCatalogItem): EffectChip[] { return effectsChipsFromItem(it); }
  imgOf(it: AuctionCatalogItem) { return normalizeImgSrc(it.imagePath); }
