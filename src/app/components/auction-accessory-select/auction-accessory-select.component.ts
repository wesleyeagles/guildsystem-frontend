// src/app/components/auction-accessory-select/auction-accessory-select.component.ts
import {
      ...
      <div class="relative" #root>
        ...
        >
          @if (selectedItem()) {
            ...
            <div class="min-w-0 flex-1 text-left">
              <div class="text-slate-300">Selecione um acessório...</div>
            </div>
          }
...
export class AuctionAccessorySelectComponent {
  ...
  set items(v: AuctionCatalogItem[]) {
    /**
     * ✅ mesmas regras do accessory select:
     * - só accessories
     * - remove quem não tem effects (sem effects não entra)
     */
    const arr = Array.isArray(v) ? v : [];
    const filtered = arr.filter((it) => it.itemType === 'accessory' && (it.effects?.length ?? 0) > 0);
  ...
  filtered = computed(() => {
    ...
    return arr.filter((it) => {
      ...
      const base = `${it.name} ${it.itemId} ${it.slot ?? ''} ${fx}`.toLowerCase();
