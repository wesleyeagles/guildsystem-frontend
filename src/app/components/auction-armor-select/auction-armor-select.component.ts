      ...
      <div class="relative" #root>
        ...
        >
          @if (selectedItem()) {
            ...
            <div class="min-w-0 flex-1 text-left"><div class="text-slate-300">Selecione uma armadura...</div></div>
          }
...
export class AuctionArmorSelectComponent {
  ...
  constructor() {
    this.query$
      .pipe(
        ...
        switchMap((q) => this.catalog.armors(q, null, 60)),
  ...
  private ensureInitialLoaded() {
    ...
    this.loading.set(true);
    this.catalog.armors('', null, 60).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (res) => {
        this.itemsSig.set(res.items ?? []);
        this.cursor.set(res.nextCursor ?? null);
        this.hasMore.set(Boolean(res.nextCursor));
  ...
  loadMore() {
    ...
    this.loading.set(true);
    this.catalog.armors(this.query(), cur, 60).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (res) => {
  ...
  selectedItem = computed(() => {
    ...
    return this.itemsSig().find((x) => x.itemType === 'armor' && x.itemId === id) ?? null;
  });
