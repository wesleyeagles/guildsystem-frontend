...
export function createPagination<T>(opts: {
  ...
  const initialCandidate = opts.initialPageSize ?? (opts.pageSizes[0] ?? 10);
  const initial = allowed.has(initialCandidate) ? initialCandidate : (opts.pageSizes[0] ?? 10);
