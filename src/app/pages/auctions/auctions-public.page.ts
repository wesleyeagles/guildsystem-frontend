// src/app/pages/auctions-public/auctions-public.page.ts
import {
...
export class AuctionsPublicPage {
    ...
    // ✅ top locked if user is the unique top holder
    ...
    // ✅ resolve itemRef do card/detalhes
    ...
    // ✅ chips para cards (lista) — todos effects do catálogo
    ...
    constructor() {
        ...
        this.timerId = setInterval(() => this.tick.set(nowMs()), 250);

        // ✅ garante catálogo carregado (pra imagem/nome/effects)
        ...
        this.socket
            ...
            .subscribe((a) => {
                ...
                if (id && d && a.id === id) {
                    ...
                    // ✅ se entrou em TIE_COUNTDOWN e não temos endsAt/participants, buscar detalhes
                    ...
                    // ✅ se saiu do desempate, limpa UI
                    if (a.status !== 'TIE_COUNTDOWN' && a.status !== 'TIE_ROLLING') {
                        const r = this.roulette();

                        // se estava em rolling, deixa a roleta ficar na tela (autoClose cuidará disso)
                        if (r.mode === 'rolling') {
                            // não faz nada
                        } else {
                            // countdown ou qualquer outro estado: pode limpar
        ...
        this.socket
            ...
            .subscribe(({ auctionId }) => {
                ...
                const r = this.roulette();
                // ✅ se a roleta estiver rodando/mostrando vencedor, NÃO fecha aqui (autoClose vai fechar)
    ...
    reload() {
        this.api.list().subscribe({
            next: (list) => this.auctions.set(list),
            error: () => { },
        });
