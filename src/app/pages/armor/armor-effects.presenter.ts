...
import type { ArmorPart, ArmorEffect } from '../../api/armor.api';
...
export class ArmorEffectsPresenter {
  hasDetails(it: ArmorPart) {
    return (Array.isArray(it.effects) && it.effects.length > 0) || Boolean(it.defense) || Boolean(it.defenseSuccesRate);
  }
  ...
  formatEffectValue(e: ArmorEffect): string {
    const label = String(e.effect ?? '').trim();
  ...
  effectsSearchText(it: ArmorPart): string {
