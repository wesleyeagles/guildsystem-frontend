...
export class ArmorTableComponent {
  private effects = inject(ArmorEffectsPresenter);
  private styles = inject(ArmorStylePresenter);

  @Input({ required: true }) items!: readonly ArmorPart[];
  @Input() isEmpty = false;

  // ✅ upgrade +0..+7 só pro Defense
  ...
  // ✅ tabela de multiplicadores igual você definiu pro Weapon
  ...
  // ✅ só aplica no defense (NÃO no defenseSuccesRate)
  displayDefense(it: ArmorPart): number {
