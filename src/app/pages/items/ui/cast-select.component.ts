  ...
  template: `
    <div class="space-y-1 mt-[8px] relative">
      <div class="flex items-center justify-between">
        <label class="text-xs text-[var(--muted)]">{{ label }}</label>
      </div>

      <div class="rounded-md border border-[var(--border)] bg-[var(--surface)] overflow-hidden">
        <button
          type="button"
          class="w-full h-[37px] px-3 py-2 text-sm flex items-center justify-between gap-3 hover:bg-[var(--surface-3)]"
          (click)="toggleOpen()"
          [disabled]="disabled"
        >
          <div class="flex items-center gap-2 min-w-0">
            @if (selectedImg()) {
              <img [src]="selectedImg()!" class="w-6 h-6 rounded-md object-cover border border-[var(--border)]" alt="" />
            }
            <div class="min-w-0">
              <div class="text-[var(--text-2)] truncate">{{ selectedName() || placeholder }}</div>
            </div>
          </div>

          <div class="text-xs text-[var(--muted)]">{{ open() ? '▲' : '▼' }}</div>
        </button>
      </div>
      ...
      @if (open()) {
        <div class="fixed inset-0 z-[220]" (click)="closeFromBackdrop($event)">
          <div
            class="absolute z-[221] rounded-md border border-[var(--border)] bg-[var(--surface)] shadow-xl overflow-hidden"
            [style.left.px]="pos().left"
            [style.top.px]="pos().top"
            [style.width.px]="pos().width"
            (click)="$event.stopPropagation()"
          >
            <div class="p-2 border-b border-[var(--border)] flex items-center gap-2">
              <input
                class="w-full rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm outline-none"
                placeholder="Buscar..."
                [(ngModel)]="q"
                (input)="applyFilter()"
                [disabled]="disabled"
              ...
              <button
                type="button"
                class="rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-xs hover:bg-[var(--surface-3)] disabled:opacity-50"
                (click)="reload()"
            ...
            <div class="max-h-[230px] overflow-auto">
              <button
                type="button"
                class="w-full px-3 py-2 text-sm text-left hover:bg-[var(--surface-3)] flex items-center gap-2"
                (click)="select(null)"
                [disabled]="disabled"
              >
                <div class="text-[var(--muted)]">— Nenhum —</div>
              </button>
              ...
              @for (c of filtered(); track c.id) {
                ...
                >
                  @if (img(c.imagePath)) {
                    <img
                      [src]="img(c.imagePath)!"
                      class="w-7 h-7 rounded-md object-cover border border-[var(--border)]"
                      alt=""
                    />
                  }
                  <div class="min-w-0">
                    <div class="text-[var(--text-2)] truncate">{{ c.name }}</div>
                  </div>
                </button>
              }

              @if (!loading() && filtered().length === 0) {
                <div class="px-3 py-6 text-center text-xs text-[var(--muted)]">Nenhum cast encontrado.</div>
              }
            </div>

            @if (error()) {
              <div class="p-2 border-t border-[var(--border)] text-xs text-red-300">{{ error() }}</div>
            }
          </div>
        </div>
      }
