...
import type { AccessoryEffect, AccessoryItem } from '../../api/accessories.api';

type UnitKind = 'none' | 'percent' | 'ms' | 'string';

// pode reaproveitar a mesma tabela do Armor (labels vêm do mesmo “effect map”)
...
export class AccessoriesEffectsPresenter {
  ...
  formatEffectValue(e: AccessoryEffect): string {
    const label = String(e.effect ?? '').trim();
  ...
  effectsSearchText(it: AccessoryItem): string {
